...
// 	.then(response => {
// 		if (!response.ok) {
// 			throw new Error(`Impossibile recuperare il Pokémon ${data[0]}. Esiste?`);
// 		}
// 		return response.json()
// 	})
// 	.then(pokeDati => {
// 		if (pokeDati.count) {
// 			throw new Error("Nessun Pokémon trovato");
// 		}
...
// variePromise('charmander', 'bulbasaur', 'celebi');
...
Promise.allSettled([
	Promise.resolve('Evviva!'),
	Promise.reject('Errore'),
	Promise.resolve('Evviva x 2!')
])
.then(res => console.log(res))
.catch(error => console.error(error));
